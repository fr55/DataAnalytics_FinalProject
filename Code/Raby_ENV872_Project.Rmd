...
In 2012, the United States Environmental Protection Agency (USEPA) established a complementary primary regulatory standards for PM2.5 at a concentration of 12 micrograms per cubic meter, ug/m3< therefore, the first statistical test should look at the standard in each station.
...
In \autoref{Coasthistplot},\autoref{Coastqqplot}, \autoref{Piedmontqqplot}, \autoref{Piedmontqqplot}, \autoref{Mountqqplot}, and \autoref{Mountqqplot} it can be seen that the data have more than one peak, heavier tails, and longer tail to the right than a normal distribution; nevertheles, environmental data often violate the assumptions of normality and the histograms fairly resembles a bell curve, so a t-test will be performed anyway.

```{r}
t.test(PM2.5_Coastal$PM2.5_Coastal, mu = 12, alternative = "less")
```

According to the One Sample t-test, NC Coastal PM 2.5 concentrations in 2018 were significanly lower than 12 ug/m3 (one sample t-test; t = --69.865, df =1754, p<0.0001).

```{r}
t.test(PM2.5_Piedmont$PM2.5_Piedmont, mu = 12, alternative = "less")
```

According to the One Sample t-test, NC Piedmont PM 2.5 concentrations in 2018 were significanly lower than 12 ug/m3 (one sample t-test; t = -67.406, df = 4433, p-value < 0.0001).

```{r}
t.test(PM2.5_Mountains$PM2.5_Mountains, mu = 12, alternative = "less")
```

According to the One Sample t-test, NC Mountains PM 2.5 concentrations in 2018 were significanly lower than 12 ug/m3 (one sample t-test; t = -53.529, df = 1270, p<0.0001).
...
leveneTest(PM2.5_Full_Elev_utm$PM2.5 ~ as.factor(PM2.5_Full_Elev_utm$Zone))
bartlett.test(PM2.5_Full_Elev_utm$PM2.5 ~ as.factor(PM2.5_Full_Elev_utm$Zone))
...
ggplot() + 
  geom_boxplot(data = PM2.5_Full_Elev_utm, aes(x = Zone,y = PM2.5, color=Zone)) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  ylab(expression("PM 2.5 Concentration (\U003BCg/m3)")) +
  xlab(expression("County")) +
  ggtitle("Exploratory Boxplot")
...
Is higher in the piedmont. Indication that population has something to do.
