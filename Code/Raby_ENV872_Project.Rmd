...
o <- data.frame("Column" = "STATE_CODE", "Description" = "A unique number identifying the State.")
...
summary_table <- rbind(a, b, c, d, e, f, t, h, i, j, k, l, m, n, o, g, p, q, r, s)
...
County_Info$Zone<-ifelse(County_Info$County == 'Ashe'
                         ...
                         ifelse(County_Info$County == 'Surry'
                                ...
                                ifelse(County_Info$County == 'Scotland'
                                       ...
                                       |County_Info$County =='Northampton'
...
Visual data exploration of the counties_sf_utm dataframe in \autoref{Countyplot} and \autoref{Zoneplot}.

```{r echo = FALSE, fig.cap = "Counties exploratory map. \\label{Countyplot}"}
ggplot() +
  annotation_map_tile(zoom = 7) +
  geom_sf(data=counties_sf_utm, aes(fill=NAME), alpha=0.7) +
  guides(fill=FALSE) +
...
ggplot() +
  annotation_map_tile(zoom = 7) +
  geom_sf(data=counties_sf_utm, aes(fill=Zone), alpha=0.7) +
...
ggplot() +
  annotation_map_tile(zoom = 7) +
  geom_sf(data = counties_sf_utm, color = 'Black', fill = "antiquewhite", alpha = 0.7) + 
...
For additional previsualization of the data and have am idea of possible trends, in Figures \autoref{PM25plot} is presented PM 2.5 daily mean concentration variation during 2018 with separate aesthetics for zone, distance to a combustion point, elevation, daily mean temperature, and population. 

```{r echo = FALSE, fig.cap = "Daily PM2.5 Concentration. \\label{PM25plot}", fig.height = 9, fig.width = 7}
PM2.5_Dist_Plot <- ggplot(PM2.5_Full_Elev_utm, aes(x=Date, y=PM2_5)) +
  geom_point(aes(color = Emiss_Dist)) +
  geom_smooth(color ="black") +
  xlab(expression("")) +
  ylab(expression("PM 2.5 (\U003BCg/m3)")) +
  scale_color_gradient(low="cyan", high="blue4", name = "Dist.comb.point (m)")  +
...
grid.arrange(PM2.5_Zone_Plot, PM2.5_Dist_Plot, PM2.5_Elevation_Plot, PM2.5_Temp_Plot, PM2.5_Pop_Plot, nrow = 5)
```
...
```{r echo = FALSE, fig.cap = "Daily PM2.5 Concentration Piedmont Histogram. \\label{Piedmonthistplot}"}
ggplot(PM2.5_Piedmont, aes(x = PM2.5_Piedmont)) +
  geom_histogram(stat = "count")
```

```{r echo = FALSE, fig.cap = "Daily PM2.5 Concentration Piedmont qqplot. \\label{Piedmontqqplot}"}
qqnorm(PM2.5_Piedmont$PM2.5_Piedmont); qqline(PM2.5_Piedmont$PM2.5_Piedmont)
```

```{r echo = FALSE, fig.cap = "Daily PM2.5 Concentration Mountains Histogram. \\label{Mounthistplot}"}
ggplot(PM2.5_Mountains, aes(x = PM2.5_Mountains)) +
  geom_histogram(stat = "count")
```

```{r echo = FALSE, fig.cap = "Daily PM2.5 Concentration Mountains qqplot. \\label{Mountqqplot}"}
qqnorm(PM2.5_Mountains$PM2.5_Mountains); qqline(PM2.5_Mountains$PM2.5_Mountains)
```

In \autoref{Coasthistplot},\autoref{Coastqqplot}, \autoref{Piedmonthistplot}, \autoref{Piedmontqqplot}, \autoref{Mounthistplot}, and \autoref{Mountqqplot} it can be seen that the data have more than one peak, heavier tails, and longer tail to the right than a normal distribution; nevertheless, environmental data often violate the assumptions of normality and the histograms fairly resembles a bell curve, so a t-test is performed anyway.
...
ggplot(PM2.5_Full_Elev_utm, aes(x=Date, y=PM2_5)) +
  geom_point(aes(color = PM2_5), alpha = 0.4) +
  geom_hline(yintercept=12, linetype="dashed", color = "black", size = 1) +
...
ggplot() + 
  geom_boxplot(data = PM2.5_Full_Elev_utm, aes(x = Zone,y = PM2_5, fill=Zone)) + 
...
In \autoref{Final_plot}} is presented the Daily PM2.5 Concentration vs Model Variables.

```{r echo = FALSE, fig.cap = "Daily PM2.5 Concentration vs Model Variables. \\label{Final_plot}", fig.height = 9, fig.width = 7}
Plot1 <- ggplot(PM2.5_Full_Elev_utm, aes(x=Population, y=PM2_5)) +
  geom_point(aes(color = PM2_5), alpha = 0.4) +
  geom_hline(yintercept=12, linetype="dashed", color = "black", size = 1) +
  geom_smooth(aes(x = Population, y = PM2_5),method = 'lm', se = FALSE) +
  annotate("text", x = 750000, y = 13.5, label = "NC regulatory standard = 12 ug/m3") +
  annotate("text", x = 600000, y = 4, label = "lm()", color = "Blue") +
  xlab(expression("County Population")) +
  ylab(expression("PM 2.5 (\U003BCg/m3)")) +
  scale_color_distiller(palette = "Set1") +
  ggtitle("2018 Daily PM2.5 Concentration vs Model Variables")

Plot2 <- ggplot(PM2.5_Full_Elev_utm, aes(x=PM10, y=PM2_5)) +
  geom_point(aes(color = PM2_5), alpha = 0.4) +
  geom_hline(yintercept=12, linetype="dashed", color = "black", size = 1) +
  geom_smooth(aes(x = PM10, y = PM2_5),method = 'lm', se = FALSE) +
  annotate("text", x = 45, y = 13.5, label = "NC regulatory standard = 12 ug/m3") +
  annotate("text", x = 47, y = 26, label = "lm()", color = "Blue") +
  xlab(expression("PM 10 Concentration (\U003BCg/m3)")) +
  ylab(expression("PM 2.5 (\U003BCg/m3)")) +
  scale_color_distiller(palette = "Set1")

Plot3 <- ggplot(PM2.5_Full_for.model, aes(x=TAVG, y=PM2_5)) +
  geom_point(aes(color = PM2_5), alpha = 0.4) +
  geom_hline(yintercept=12, linetype="dashed", color = "black", size = 1) +
  geom_smooth(aes(x = TAVG, y = PM2_5),method = 'lm', se = FALSE) +
  annotate("text", x = 40, y = 13.5, label = "NC regulatory standard = 12 ug/m3") +
  annotate("text", x = 25, y = 6, label = "lm()", color = "Blue") +
  xlab(expression("Daily Average Temperature (°F)")) +
  ylab(expression("PM 2.5 (\U003BCg/m3)")) +
  scale_color_distiller(palette = "Set1")

Plot4 <- ggplot(PM2.5_Full_Elev_utm, aes(x=elevation, y=PM2_5)) +
  geom_point(aes(color = PM2_5), alpha = 0.4) +
  geom_hline(yintercept=12, linetype="dashed", color = "black", size = 1) +
  geom_smooth(aes(x = elevation, y = PM2_5),method = 'lm', se = FALSE) +
  annotate("text", x = 1000, y = 13.5, label = "NC regulatory standard = 12 ug/m3") +
  annotate("text", x = 1200, y = 4, label = "lm()", color = "Blue") +
  xlab(expression("Elevation (meters)")) +
  ylab(expression("PM 2.5 (\U003BCg/m3)")) +
  scale_color_distiller(palette = "Set1")
